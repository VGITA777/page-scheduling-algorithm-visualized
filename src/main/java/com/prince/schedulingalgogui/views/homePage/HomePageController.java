...
import com.prince.schedulingalgogui.schedulingAlgorithms.FifoScheduler;
import com.prince.schedulingalgogui.schedulingAlgorithms.ISchedulerFactory;
import com.prince.schedulingalgogui.schedulingAlgorithms.LruScheduler;
import com.prince.schedulingalgogui.schedulingAlgorithms.PageResultStatus;
import com.prince.schedulingalgogui.schedulingAlgorithms.Scheduler;
...
public class HomePageController implements Initializable {
  ...
  private void updateDescriptiveResults(@NonNull SchedulerResult schedulerResult) {
    ...
    hitPercentageText.setText(String.format("%.2f",
        calculateHitPercentage(schedulerResult.getPageHit(), schedulerResult.getPageFault()).isNaN()
            ? 0 : calculateHitPercentage(schedulerResult.getPageHit(),
            schedulerResult.getPageFault())));
    faultPercentageText.setText(String.format("%.2f",
        calculateFaultPercentage(schedulerResult.getPageHit(),
            schedulerResult.getPageFault()).isNaN() ? 0
            : calculateFaultPercentage(schedulerResult.getPageHit(),
                schedulerResult.getPageFault())));
  }
